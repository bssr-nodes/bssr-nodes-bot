...
exports.run = async (client, message, args) => {
    ...
    for (let i = 0; i < Proxies.length; i++) {
        ...
        if (!idOfProxy) {
            ...
            if (deletedObject) {
                ReplyMessage.edit(
                    `The domain should now work, please ensure there is a DNS record pointing to the BSSR Nodes proxy and Cloudflare proxy is disabled if you are using Cloudflare.`,
                );
            } else {
                ReplyMessage.edit(
