...
exports.run = async (client, message, args) => {
    ...
    if (message.member.roles.cache.find((r) => r.id === "1250045509868195840")) userID = args[1] || message.author.id;
    ...
    axios({
        ...
        if (arr.length == 0) {
            ...
            if (freeServers.length > 0) serverListEmbed.addField(`Servers (${freeServers.length})`, freeServers.join(", "));
